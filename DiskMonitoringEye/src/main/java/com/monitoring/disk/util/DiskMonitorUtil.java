...
public class DiskMonitorUtil {
    ...
    public static DiskRawInfo getDiskRawInfo() {
        ...
        try {
            if (osNameLower.contains("win")) {
                return getWindowsDiskInfo(osName);
            } else if (osNameLower.contains("nix") || osNameLower.contains("nux") || osNameLower.contains("mac")) {
                return getLinuxDiskInfo(osName);
            } else if (osNameLower.contains("sunos") || osNameLower.contains("solaris")) {
                return getSolarisDiskInfo(osName);
            } else {
    ...
    private static DiskRawInfo getLinuxDiskInfo(String osName) throws Exception {
        Process process = Runtime.getRuntime().exec(new String[]{"df", "-P"});
        try {
     ...
     */
    private static DiskRawInfo getSolarisDiskInfo(String osName) throws Exception {
        Process process = Runtime.getRuntime().exec(new String[]{"df", "-k"});
        try {
            try (BufferedReader reader = new BufferedReader(
                    new InputStreamReader(process.getInputStream(), StandardCharsets.UTF_8))) {
                reader.readLine(); // 헤더 무시
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    try {
                        // capacity 컬럼(5번째)을 직접 읽어 바로 임계치 검사
                        double usage = Double.parseDouble(parts[4].replace("%", ""));
                        ...
                        // 대시보드 반환용: 가장 높은 사용률 파일시스템의 상세 정보만 파싱
                        if (usage > highest.usagePercentage()) {
                            long totalKb = Long.parseLong(parts[1]);
                            long usedKb  = Long.parseLong(parts[2]);
                            long freeKb  = Long.parseLong(parts[3]);
